...
echo "access the dashboard at https://$(kubectl --context ${mgmt_context} get svc -n istio-gateways --selector=istio=mgmt-ingressgateway -o jsonpath='{.items[*].status.loadBalancer.ingress[0].ip}{.status.loadBalancer.ingress[0].hostname}')/solo"
echo
