...
echo "Load Balancer IP: https://$(kubectl --context ${mgmt_context} get svc -n gloo-system --selector=gloo=kube-gateway -o jsonpath='{.status.loadBalancer.ingress[0].ip}{.status.loadBalancer.ingress[0].hostname})/solo"
echo
