...
echo "Load Balancer IP: https://$(kubectl --context ${mgmt_context} get svc -n gloo-system --selector=gloo=kube-gateway -o jsonpath='{.items[*].status.loadBalancer.ingress[0].*}')/solo"
echo
