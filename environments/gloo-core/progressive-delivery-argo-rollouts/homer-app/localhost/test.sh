...
echo "access the dashboard at https://$(kubectl get svc -n istio-gateways --selector=istio=ingressgateway -o jsonpath='{.items[*].status.loadBalancer.ingress[0].ip}')"
echo
...
echo "https://github.com/solo-io/aoa-catalog/tree/main/environments/gloo-core/single-cluster-bookinfo#application-description"
echo
echo