...
echo "$(kubectl -n gloo-system get service gateway-proxy -o jsonpath='{.status.loadBalancer.ingress[0].ip}') argocd-local.glootest.com gmui-local.glootest.com backstage-local.glootest.com"
echo "EOF"
echo
echo "access argocd at https://argocd-local.glootest.com/argo"
echo "access Gloo Mesh UI at https://gmui-local.glootest.com"
echo "access the backstage application at: https://backstage-local.glootest.com/productpage"
echo
