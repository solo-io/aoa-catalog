...
The `gloo-gateway/single-cluster-backstage` environment deploys the core components of a single cluster Gloo Platform demo, with backstage deployed and exposed using Istio. The image used for Backstage is currently the default with no plugins, if you have a pre-built image with desired plugins, modify the Backstage ArgoCD Application helm values. 

![High Level Architecture](.images/single-cluster-backstage-arch-1a.png)
...
${GATEWAY_IP} argocd-local.glootest.com gmui-local.glootest.com backstage-local.glootest.com
EOF
```
...
```
cat <<EOF | sudo tee -a /etc/hosts
localhost argocd-local.glootest.com gmui-local.glootest.com backstage-local.glootest.com
EOF
```
