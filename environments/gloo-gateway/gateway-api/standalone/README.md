...
The RouteTables for applications exposed in this demo are defining non-wildcard hosts which follow the pattern `<app>.edge.glooplatform.com`. You can map these hostnames to your gateway IP address in your DNS service of choice (i.e. Route53, Cloudflare), or you can follow the methods below to modify your `/etc/hosts` locally depending on your cluster LoadBalancer configuration.

Applications Exposed using this demo:
- ArgoCD at `https://argocd.edge.glooplatform.com/argo`
    - argocd credentials:
    - user: admin
    - password: solo.io
- bookinfo at `https://bookinfo.edge.glooplatform.com/productpage`
- httpbin at `https://httpbin.edge.glooplatform.com/get`
    - okta credentials:
    - user: jdoe@solo.io
    - password: gloo-admin
...
```
GATEWAY_IP=$(kubectl -n gloo-system get service gateway-proxy -o jsonpath='{.status.loadBalancer.ingress[0].ip}{.status.loadBalancer.ingress[0].hostname}')
...
${GATEWAY_IP} argocd.edge.glooplatform.com httpbin.edge.glooplatform.com bookinfo.edge.glooplatform.com
EOF
```
...
```
cat <<EOF | sudo tee -a /etc/hosts
127.0.0.1 argocd.edge.glooplatform.com httpbin.edge.glooplatform.com bookinfo.edge.glooplatform.com
EOF
```
