...
The `gloo-gateway/single-cluster-httpbin` environment deploys the core components of a single cluster Gloo Platform demo, along with the httpbin sample application.

![High Level Architecture](.images/single-cluster-httpbin-arch-1a.png)
...
- Wave 5 - Configures initial Gloo Mesh components
    ...
    - expose Gloo Mesh UI
    - expose ArgoCD UI
- Wave 6 - Deploys httpbin Application
- Wave 6 - Configures httpbin Gloo Platform Config
...
${GATEWAY_IP} argocd-local.glootest.com gmui-local.glootest.com httpbin-local.glootest.com
EOF
```
...
```
cat <<EOF | sudo tee -a /etc/hosts
127.0.0.1 argocd-local.glootest.com gmui-local.glootest.com httpbin-local.glootest.com
EOF
```
