...
echo "access the dashboard at https://$(kubectl get svc -n gloo-system --selector=gloo=gateway-proxy -o jsonpath='{.items[*].status.loadBalancer.ingress[0].ip}{.status.loadBalancer.ingress[0].hostname}')/solo"
echo