...
The `gloo-edge-solo-dev-portal` environment deploys a Gloo Edge, Gloo Portal, and a Solo.io branded Portal displaying the httpbin and petstore applications along with configured Portal OIDC authentication, pre-configured user(s), and Try It Now! capabilities with Usage Plans
...
```
cat <<EOF | sudo tee -a /etc/hosts
localhost argocd-local.glootest.com api-local.glootest.com apidev-local.glootest.com portal-local.glootest.com
EOF
```
