...
echo "$(kubectl -n gloo-system get service gateway-proxy -o jsonpath='{.status.loadBalancer.ingress[0].ip}') argocd.glootest.com bookinfo.glootest.com httpbin.glootest.com"
echo "EOF"
echo
echo "Method 2 - K3d LB Integration: modify /etc/hosts on your local machine (this will require sudo privileges)"
echo "cat <<EOF | sudo tee -a /etc/hosts"
echo "127.0.0.1 argocd.glootest.com bookinfo.glootest.com httpbin.glootest.com"
echo "EOF"
echo
echo "access argocd at https://argocd.glootest.com/argo"
echo "access the bookinfo application at: https://bookinfo.glootest.com/productpage"
echo "access the httpbin application at: https://httpbin.glootest.com/get"
echo
