...
The `gm-single-cluster-solowallet` environment deploys the core components of a single cluster Gloo Platform demo, along with the solowallet sample application.
...
```
cat <<EOF | sudo tee -a /etc/hosts
localhost argocd-local.glootest.com gmui-local.glootest.com solowallet-local.glootest.com
EOF
```
