...
The `gloo-edge-flagger` environment deploys Gloo Edge along with flagger and podinfo applications to demonstrate progressive delivery capabilities
...
- ArgoCD at `https://argocd-local.glootest.com/argo`
    ...
    - user: admin
    - password: solo.io
- podinfo at `https://podinfo-local.glootest.com/productpage`
...
${GATEWAY_IP} argocd-local.glootest.com podinfo-local.glootest.com
EOF
```
...
```
cat <<EOF | sudo tee -a /etc/hosts
localhost argocd-local.glootest.com podinfo-local.glootest.com
EOF
```
