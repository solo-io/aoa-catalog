...
LB_ADDRESS=$(kubectl get svc --context ${cluster_context}  -n istio-gateways --selector=istio=ingressgateway -o jsonpath='{.items[*].status.loadBalancer.ingress[0].ip}{.status.loadBalancer.ingress[0].hostname}')
