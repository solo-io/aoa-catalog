...
LB_ADDRESS=$(kubectl get svc --context ${cluster_context}  -n istio-gateways --selector=istio=ingressgateway -o jsonpath='{.items[*].status.loadBalancer.ingress[0].ip}{.items[*].status.loadBalancer.ingress[0].hostname}')
              ...
              config.yml: |
                ...
                services:
                  ...
                  - name: "Web Applications"
                    icon: "fas fa-cloud"
                    items:
                      - name: "Argo Rollouts Demo Application"
                        subtitle: "modify image tag in ArgoCD"
                        icon: "fas fa-globe-americas"
                        tag: "podinfo"
                        url: "https://$LB_ADDRESS"
                        target: "_blank" # optional html a tag target attribute 
                      - name: "Helloworld Rollouts Application"
                        subtitle: "modify image tag in ArgoCD"
                        icon: "fas fa-globe-americas"
                        tag: "helloworld"
                        url: "https://$LB_ADDRESS/hello"
                        target: "_blank" # optional html a tag target attribute 
