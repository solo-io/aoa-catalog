...
echo "access the dashboard at https://$(kubectl --context ${cluster_context} get svc -n istio-gateways --selector=istio=ingressgateway -o jsonpath='{.items[*].status.loadBalancer.ingress[0].ip}{.status.loadBalancer.ingress[0].hostname}')/solo"
echo
